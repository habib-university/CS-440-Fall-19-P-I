...
// translate a point with given tranlation matrix
function translatePoint(matrix, point){
    var result = vec4(0, 0, 0, 0);
    var rows = matrix.length;

    for( var i = 0; i < rows; i++){
        var m_row = vec4(matrix[i][0], matrix[i][1], matrix[i][2], matrix[i][3] );
        result[i] = dot(m_row, point);
    }
...
//----------------------------------------------------------------------------
// Mix takes in two vectors and a number (alpha) and calculates the X coordinate
function mix( u, v, s )
{
    if ( typeof s !== "number" ) {
...
/*
* P and Q are vectors, with X lying on the line segment PQ
* A and B are vectors
* Using P,Q, and X, find X' that lies on AB
*/
function map_point(P, Q, A, B, X)
{
    ...
    var alpha = (X_norm-P_norm)/(Q_norm-P_norm);
    var X_prime = mix(A, B, alpha);
